...
void list_map_mut(struct list* l, int64_t (f) (int64_t))  {
    while(l != NULL) {
        l->value = f(l->value); // l value is result of execution f function with l value parameter
        l = l->next; // move to next iteration
    }
}
...
void list_triple(struct list* l) { 
    list_map_mut(l, &triple); // using described logic
}